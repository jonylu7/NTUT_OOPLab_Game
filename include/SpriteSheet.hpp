...
public:
    SpriteSheet() {}
    void Start(std::shared_ptr<Util::Image> image, int spriteWidth,
        ...
        for (int i = 0; i < numSpirtes; i++) {
            float topY = (currentY + spriteHeight) /
                         (float)m_SpriteSheet_Image->getHeight();
            float rightX = (currentX + spriteWidth) /
                           (float)m_SpriteSheet_Image->getWidth();
            float leftX = currentX / (float)m_SpriteSheet_Image->getWidth();
            float bottomY = currentY / (float)m_SpriteSheet_Image->getHeight();

            std::vector<glm::vec2> texCoords = {
                glm::vec2(rightX, topY), glm::vec2(rightX, bottomY),
                glm::vec2(leftX, bottomY), glm::vec2(leftX, topY)};
