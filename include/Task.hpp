...
public:
    ...
    ~Task() override {}
    void Start(std::shared_ptr<CameraClass> camera) { m_Camera = camera; }
    void setObjectScale(float scale) { m_Transform.scale = {scale, scale}; }
    void Draw() override {
        if (!m_Visible) {
            return;
        }
        if (b_MovingRelativeToCamera) {
            ...
            // {m_Camera->getPosition().x*CAM_OFFSET,m_Camera->getPosition().y*CAM_OFFSET};
            // printf("(Task): pos =
            // {%5f,%5f}",m_Transform.translation.x,m_Transform.translation.y);
