...
public:
    EnemyScripts(){};
    ~EnemyScripts(){};
    void Start(std::shared_ptr<EnemyPlayer> enemyPlayer,std::shared_ptr<UnitManager> GameObjectManager,std::shared_ptr<MapClass> map){
    ...
    void UpdateSpawnScript(){
        if(m_Enemy->getUnitConstructCount(UnitType::POWER_PLANT)<1 && m_Enemy->getTotalCurrency()> 300){
            m_GameObjectManager->spawn(m_Map,UnitType::POWER_PLANT ,HouseType::ENEMY,{m_baseCell.x+constructCountX,m_baseCell.y+constructCountY});
            constructCountX+=SPACE;
            CDTime=15.f*CHEAT;
        }else if(m_Enemy->getUnitConstructCount(UnitType::ORE_REF)<1 && m_Enemy->getTotalCurrency()> 2000){
            m_GameObjectManager->spawn(m_Map,UnitType::ORE_REF ,HouseType::ENEMY,{m_baseCell.x+constructCountX,m_baseCell.y+constructCountY});
            constructCountX+=SPACE;
            CDTime=100.f*CHEAT;
        }else if(m_Enemy->getUnitConstructCount(UnitType::BARRACKS)<1 && m_Enemy->getTotalCurrency()> 300){
            m_GameObjectManager->spawn(m_Map,UnitType::BARRACKS ,HouseType::ENEMY,{m_baseCell.x+constructCountX,m_baseCell.y+constructCountY});
            constructCountX+=SPACE;
            CDTime=15.f*CHEAT;
        }else if(m_Enemy->getAvatarCount()<=25 && m_Enemy->getTotalCurrency()> 100){
            m_GameObjectManager->spawn(m_Map,UnitType:: INFANTRY,HouseType::ENEMY);
            CDTime=5.f*CHEAT;
        }else if(m_Enemy->getUnitConstructCount(UnitType::WAR_FACT)<1 && m_Enemy->getTotalCurrency()> 2000){
            m_GameObjectManager->spawn(m_Map,UnitType:: WAR_FACT,HouseType::ENEMY,{m_baseCell.x+constructCountX,m_baseCell.y+constructCountY});
            constructCountX+=SPACE;
            CDTime=100.f*CHEAT;
        }else if(m_Enemy->getUnitConstructCount(UnitType::ADV_POWER_PLANT)<1 && m_Enemy->getTotalCurrency()> 500){
            m_GameObjectManager->spawn(m_Map,UnitType:: ADV_POWER_PLANT,HouseType::ENEMY,{m_baseCell.x+constructCountX,m_baseCell.y+constructCountY});
            constructCountX+=SPACE;
            CDTime=25.f*CHEAT;
        }
