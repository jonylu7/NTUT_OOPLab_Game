...
private:
    ...
    void InitGrid(){
        ...
        for (float i = (WINDOW_WIDTH / 2); i >= float(-(WINDOW_WIDTH / 2));
             i -= cellHeight) {
            // vertical
            lineV.push_back(Line(glm::vec2(i/(WINDOW_HEIGHT / 2), float(-(WINDOW_HEIGHT / 2))),
                                        glm::vec2(i/(WINDOW_HEIGHT / 2), float(WINDOW_HEIGHT / 2))));
        }

        for (float i = (WINDOW_HEIGHT / 2); i >= -float(WINDOW_HEIGHT / 2);i -= cellWidth) {
            // horz
            lineV.push_back(Line(glm::vec2(float(-(WINDOW_WIDTH / 2)/(WINDOW_WIDTH / 2)), i/(WINDOW_HEIGHT / 2)),
                                        glm::vec2(float((WINDOW_WIDTH / 2)/(WINDOW_WIDTH / 2)), i/(WINDOW_HEIGHT / 2))));
        }
        m_Grid.Start(lineV);
    }
